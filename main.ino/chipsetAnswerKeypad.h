...
char chipset_input[3];             // Char array to store 1-2 digits - user_input[0]-[2]
extern byte digitCount;            // To track the number of digits entered

char* processKeypadChipset() 
{
  digitCount = 0;                                        //Number of digits pressed by user
  lcd.setCursor(17, 3);                                   //LCD --> (COLUMN, ROW)
  ...
  while (true) {
    ...
    if (customKey) {
      if (customKey >= '0' && customKey <= '9') {
        ...
        if ( digitCount < 3) {
          chipset_input[digitCount] = customKey;            // Store the digit in user_input
          lcd.setCursor(17 + digitCount, 3);              // Move the cursor based on the digits
        ...
        chipset_input[digitCount] = '\0';         // Null-terminate the input string

        // Return the entered input to the main function
        return chipset_input;
      } else if (customKey == '*') {
        // If the '*' key (Backspace) is pressed
        if (digitCount > 0) {
          digitCount--;                           // Move back in the input array
          chipset_input[digitCount] = '\0';               // Null out the last character
          
          // Update the LCD to reflect the change
          lcd.setCursor(17 + digitCount, 3);        // Set the cursor at the right position
          lcd.print(" ");                          // Clear the last character on the LCD
          lcd.setCursor(17 + digitCount, 3);       // Move cursor back to the cleared position
        }
      }
    }
  }
